...
```
This graph shows the distribution of salaries according to the number of seasons in the NBA and the trend that these values follow. We can see that the salary of the NBA players increases until 10 seasons and then it starts to decrease.

##### Nº Seasons x Salary x Experience in other Leagues
...
ggplot() +
  ...
  theme(legend.position="right",
        plot.title = element_text(hjust = 0.5))
```
If we split the first graph by the Experience in other Leagues (usually foreign players) We can observe that players with experience in other leagues have a higher salary than those who have played in other leagues. The trend seems to be the same in both cases but in foreign players we can see that the salary decrease comes earlier (it makes sense considering that these players come to the NBA older and play fewer seasons in this competition that the national players)

##### Minutes x Points x Salary
```{r, echo=FALSE, fig.width = 12}
ggplot() +
...
ggplot() +
  ...
  xlab('Minutes played')
```
This graph shows how performance and salary evolves over the seasons. It is clear that there are more players with few seasons (between 1 and 6) and that they are the ones who earns less money. As they have more experience their salary increases along with their performance, because they play more minutes and the points/minute ratio follows a much steeper trend.
